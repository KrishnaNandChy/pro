...
const generateFileName = (userId, allPostsLength) => {
  ...
  return `${userId}-${allPotsLength}-${date}.png`;
};
...
const createPostWithImagesController_V2 = async (req, res, next) => {
  const { userId } = req.params;
  const { prompt, negativePrompt, size, n, imageURL } = req.body;
  ...
  try {
    ...
    const downloadAndConvertImages = await promises.all(
...
const getPostsController = async (req, res, next) => {
  try {
    const allPosts = await Post.find().population("user", "username");
